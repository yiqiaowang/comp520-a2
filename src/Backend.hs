...
-- Assignment compatibility. Check if Type A can be assigned the value of Type B
...
-- S_If without else block, so much for DRY
...
-- Type Check Expressions. Only takes in one expression, not a list like before
typeCheckExpr :: Map IdName Entry -> Expr -> (Type, Bool)

-- leaf nodes, automatically correctly typed
typeCheckExpr m (E_Int _) = (T_Int, True)
typeCheckExpr m (E_Float _) = (T_Float, True)
typeCheckExpr m (E_String _) = (T_String, True)

-- Other expressions, are type correct if their arguments are type correct
-- and if their arguments can be resolved to a single type (dependent of the operation)
...
typeCheckExpr m (E_Paren a) = typeCheckExpr m a
...
preAmble =
  "char* concat(char *str1, char *str2){\
	   ...
	   \strcpy(str3, str1);\
           ...
           \if (n < 0) {printf(\"%s\",\"Error, String * Negative Int\"); exit(1);}\
...
getIdType i m = parseEntry $ removeJust $ getSym i m
...
codeGenStmt m (S_Assign (E_Iden i) e)
  | t == T_String && (not $ isIden e) =
    "strcpy(" ++ i ++ "," ++ codeGenExpr m e ++ ");"
