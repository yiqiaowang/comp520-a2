...
module Backend (initSymTbl, typeCheckWrap) where
...
-- S_PRINT
typeCheck m (S_Print (E_Iden id) : ss) = (hasKey id m) && (typeCheck m ss)

-- S_ASSIGN
typeCheck m (S_Assign a b: ss) = ((fst $ typeCheckExpr m a) == (fst $ typeCheckExpr m b)) && (typeCheck m ss)

-- S_if
typeCheck m (S_If a b e : ss)
  | (elseBlock e) = (snd $ typeCheckExpr m a) && (typeCheck m b) && (typeCheck m $ extractElse e) && (typeCheck m ss)
  | not (elseBlock e) = (snd $ typeCheckExpr m a) && (typeCheck m b) && (typeCheck m ss)

-- S_While
typeCheck m (S_While a b : ss) = (snd $ typeCheckExpr m a) && (typeCheck m b) && (typeCheck m ss)
...
typeCheckExpr m (E_Div a b)
  | ((snd $ typeCheckExpr m a) && (snd $ typeCheckExpr m b)) = (resolveType Op_Div (fst $ typeCheckExpr m a) (fst $ typeCheckExpr m b), True)
  | otherwise = error "Type Error"
typeCheckExpr m (E_Add a b)
  | ((snd $ typeCheckExpr m a) && (snd $ typeCheckExpr m b)) = (resolveType Op_Add (fst $ typeCheckExpr m a) (fst $ typeCheckExpr m b), True)
  | otherwise = error "Type Error"
typeCheckExpr m (E_Mult a b)
  | ((snd $ typeCheckExpr m a) && (snd $ typeCheckExpr m b)) = (resolveType Op_Mult (fst $ typeCheckExpr m a) (fst $ typeCheckExpr m b), True)
  | otherwise = error "Type Error"
typeCheckExpr m (E_Sub a b)
  | ((snd $ typeCheckExpr m a) && (snd $ typeCheckExpr m b)) = (resolveType Op_Sub (fst $ typeCheckExpr m a) (fst $ typeCheckExpr m b), True)
  | otherwise = error "Type Error"
typeCheckExpr m (E_UMinus a)
  | ((unaryCompat $ fst $ typeCheckExpr m a) && (snd $ typeCheckExpr m a)) = (fst $ typeCheckExpr m a, True)
  | otherwise = error "Type Error"
