...
preAmble = "char* concat(char *str1, char *str2){\
...
codeGenDecl (D_Decl (E_Iden i) t)
  ...
  | (t == T_Int) = "int " ++ i ++ " = 0;"
  | (t == T_String) = "char* " ++ i ++ " = (char *) malloc(1024); strcpy(" ++ i ++ ",\"\");"
...
codeGenStmt m (S_Print e)
  ...
  where t = fst $ typeCheckExpr m e

codeGenStmt m (S_Assign (E_Iden i) e) = i ++ " = " ++ codeGenExpr m e ++ ";"
codeGenStmt m (S_If a b B_Endif) = "if (" ++ codeGenExpr m a ++ ") {" ++ codeGenStmtWrap m b ++ "};"
codeGenStmt m (S_If a b (B_Else c)) = "if (" ++ codeGenExpr m a ++ ") {" ++ codeGenStmtWrap m b ++ "} else {" ++ codeGenStmtWrap m c ++ "};"
codeGenStmt m (S_While a b) = "while (" ++ codeGenExpr m a ++ ") {" ++ codeGenStmtWrap m b ++ "};"
...
codeGenExpr m (E_Add a b)
  | (t == T_String) = "(concat(" ++ codeGenExpr m a ++ "," ++ codeGenExpr m b ++ "))"
  | otherwise = "(" ++ codeGenExpr m a ++ "," ++ codeGenExpr m b ++ ")"
  where t = (fst $ resolveType Op_Add (fst $ typeCheckExpr m a) (fst $ typeCheckExpr m b))
codeGenExpr m (E_Sub a b) = "(" ++ codeGenExpr m a ++ " - " ++ codeGenExpr m b ++ ")"
codeGenExpr m (E_Mult a b)
  | (t == T_String) && (x == T_Int) && (y == T_String) = "(str_mult(" ++ codeGenExpr m b ++ "," ++ codeGenExpr m a ++ "))"
  | (t == T_String) && (x == T_String) && (y == T_Int) = "(str_mult(" ++ codeGenExpr m a ++ "," ++ codeGenExpr m b ++ "))"
  | otherwise = "(" ++ codeGenExpr m a ++ " * " ++ codeGenExpr m b ++ ")"
  where t = (fst $ resolveType Op_Mult (fst $ typeCheckExpr m a) (fst $ typeCheckExpr m b))
        x = (fst $ typeCheckExpr m a)
        y = (fst $ typeCheckExpr m b)
codeGenExpr m (E_Div a b) = "(" ++ codeGenExpr m a ++ " / " ++ codeGenExpr m b ++ ")"
