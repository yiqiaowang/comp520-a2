...
codeGenWrap p m 
  | typeCheckWrap p m = codeGen p
  | otherwise = error "You shouldn't be reading this tbqh." 
  
codeGen :: Prog -> String
codeGen Prog d s = codeGenDecl d ++ codeGenStmt s 
...
codeGenDecl D_Decl (E_Iden i) t = typeToStr t ++ " " ++ i ++ " = " ++ typeInitVal t
