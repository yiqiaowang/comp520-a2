...
typeCheck m (S_If a b e:ss)
  | (elseBlock e) =
    ...
    (typeCheck m b) && (typeCheck m $ extractElse e) && (typeCheck m ss)
  | not (elseBlock e) =
...
typeCheckExpr m (E_Div a b)
  | ((snd $ typeCheckExpr m a) && (snd $ typeCheckExpr m b)) =
    ( resolveType Op_Div (fst $ typeCheckExpr m a) (fst $ typeCheckExpr m b)
    , True)
...
typeCheckExpr m (E_Sub a b)
  | ((snd $ typeCheckExpr m a) && (snd $ typeCheckExpr m b)) =
    ( resolveType Op_Sub (fst $ typeCheckExpr m a) (fst $ typeCheckExpr m b)
    , True)
  | otherwise = error "Type Error"
